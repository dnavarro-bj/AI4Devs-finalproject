...
* **`catalogo-especies`**: las pantallas del catálogo de especies sobre el kit —`/species`, su ficha, el alta y la corrección compartiendo formulario, y la retirada con su `409`—. Su CRUD llevaba en verde desde T-08 y el frontend **solo usaba el listado**, para poblar el selector del alta de planta: la historia 0.6 estaba sin cumplir en la interfaz. Nace `src/features/species/` con su service completo. Implementado y en verde.
...
* **`catalogo-localizaciones`**: el catálogo de localizaciones completo —`GET/PUT/DELETE /locations/{id}`, con el recuento de ejemplares en la ficha y `409` al retirar una que alberga alguno: una planta no puede quedarse sin sitio— más sus dos pantallas. `Location.rename` existía desde `invariantes-de-dominio` y nunca se había expuesto. `GET /locations` trae además el recuento por fila —una consulta agregada, no una por fila—, porque el mapa del vivero **es** la carga de cada sitio. Las dos pantallas reproducen la composición del prototipo: mapa como árbol y vista general con tarjetas de zona en el catálogo; portada, fila de métricas y dos columnas en la ficha. **La jerarquía no se anticipa en el esquema pero la composición sí se construye**: el árbol tiene un nivel y los que faltan van marcados con T-18 dentro del propio mapa. Implementado y en verde.
* **`catalogo-etiquetas`** (cierra **T-13**): el catálogo de etiquetas completo —`GET/PUT/DELETE /tags/{id}` y `POST /tags/{id}/merge`, con el recuento por fila en el listado— más sus dos pantallas. La **combinación** se resuelve en la tabla de unión con dos sentencias nativas —descartar las asignaciones que colisionarían y reasignar el resto— y no recorriendo las plantas: `plant_tag` tiene clave primaria compuesta, así que reasignar a ciegas una planta que ya tiene ambas la violaría. La respuesta dice a cuántas plantas alcanzó, y la pantalla declara la cifra **antes** de confirmar. Retirar una etiqueta en uso responde `409` y ofrece combinarla en lugar de dejar sin salida. Implementado y en verde.
...
* **El kit son 36 componentes** en `app/components/ui/`: acciones, campos, acción alineada con los campos, estado, prioridad, filtro, barra de filtros, panel, tabla, paginación, métrica navegable, resumen de estado, árbol, cronología, agenda, calendario mensual, pauta anual, galería, zona de subida, sección de formulario, navegación de secciones, rango de meses, barra de proporciones, rueda de proporción, escala con rango, aviso, diálogo, toast, breadcrumbs, grupo de navegación, pestañas, cabecera de página, búsqueda global, estado vacío, error en línea y etiqueta de ejemplar. (`UiTreeNode` no cuenta aparte: es la parte recursiva de `UiTree`.)
...
* **Una pantalla construida reproduce la composición de su `data-screen` del [prototipo](docs/wireframes/README.md)**, no solo sus datos: qué bloque la abre, qué va como figura y no como cifra, y qué ocupa la columna principal frente a la lateral. Lo que el prototipo compone y el API todavía no alimenta va **marcado con su ticket en su sitio del layout**, nunca omitido: un bloque que falta no se distingue de un olvido y obliga a rehacer la pantalla cuando llegue su ticket, en vez de rellenarla. Los datos correctos en una composición equivocada son una pantalla mal entregada —pasó con mezclas (`sustratos-como-el-wireframe`) y con localizaciones—. Las reglas están en `openspec/config.yaml`, para que cada change las lea al proponerse y al implementarse.
